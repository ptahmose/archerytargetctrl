...
class TargetSegment {
    constructor(radius, marginWidth, text, segmentColor, marginColor, textColor) {
        this.radius = radius;
...
class TargetCtrl {
    ...
    setupEvents() {
        this.element.onmousedown = (ev) => { this.OnMouseDown(ev); };
        this.element.onmouseup = (ev) => { this.OnMouseUp(ev); };
        this.element.onmousemove = (ev) => { this.OnMouseMove(ev); };
        this.element.addEventListener("touchstart", (ev) => { this.OnTouchStart(ev); }, false);
        this.element.addEventListener("touchmove", (ev) => { this.OnTouchMove(ev); }, false);
    ...
    // }
    setTransform(ctx, centerX, centerY, zoom) {
        zoom = 1 / zoom;
    ...
    TransformToUnzoomedNormalized(pos) {
    ...
    runZoomInAnimation(ev, startZoom, endZoom) {
        this.runZoomInAnimation_(this.getMousePos(this.element, ev), startZoom, endZoom);
    }
    runZoomInAnimation_(zoomCenter, startZoom, endZoom) {
        //var pos = this.getMousePos(this.element, ev);
    ...
    drawZoomed(ctx, centerX, centerY, zoom) {
    ...
    paintSegment(ctx, startRadius, endRadius, centerX, centerY) {
    ...
    paintSegmentTs(ctx, canvasInfo, startRadius, endRadius, color) {
        ctx.beginPath();
